...
function Customrow(props) {
  ...
  return (
    <>
      <TableRow key={props.row.leave_id}>
        <TableCell>
          {"" + format(new Date(props.row["Apply Date"]), "EEE, dd MMM yyyy")}
        </TableCell>
        <TableCell>{props.row.Type}</TableCell>
        <TableCell>
          {"" + format(new Date(props.row["Start Date"]), "EEE, dd MMM yyyy")}
        </TableCell>
        <TableCell>
...
function LeaveTable(props) {
  ...
  if (props.leaves !== null) {
    rows = [...props.leaves.Leaves].reverse();
  }
          ...
          <TableBody>
            {rows !== null
              ? rows
                  ...
                  .map((row) => <Customrow row={row} key={row.leave_id} />)
              : null}
          </TableBody>
        </Table>
        <TablePagination
